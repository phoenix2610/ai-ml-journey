"""Gemini client for the agent: generation and tool calling.
...
def load_key(explicit: str | None = None) -> str:
    ...
    raise MissingKey(
        ...
        "agent.llm.load_dotenv, but not by most other .env readers."
    )
...
class GeminiClient:
    ...
    def generate_with_tools(
        ...
        *,
        ...
        Returns ``{"text": str | None, "calls": [{"name", "args"}]}``. A turn
        may contain both -- the model can narrate and call a tool at once.

        Temperature defaults to 0: tool selection is a decision, not a
        creative act, and sampling variance here shows up as wrong arguments.
        """
        payload: dict[str, Any] = {
            "contents": messages,
            "generationConfig": {
        ...
        if tool_schemas:
            payload["tools"] = [{"functionDeclarations": tool_schemas}]
    ...
    def _extract_turn(self, data: dict) -> dict:
        meta = data.get("usageMetadata") or {}
        self.usage.add(
            int(meta.get("promptTokenCount", 0)), int(meta.get("candidatesTokenCount", 0))
        )
        ...
        if not candidates:
            blocked = (data.get("promptFeedback") or {}).get("blockReason")
            raise LLMError(f"Gemini returned no candidates (blocked: {blocked})")
        ...
        for part in parts:
            ...
            if "functionCall" in part:
                ...
                calls.append({
                    ...
                    # Gemini 3.x rejects a replayed functionCall that has lost
                    # its thought signature, so it has to survive the round trip.
...
def model_message(text: str | None, calls: list[dict] | None = None) -> dict:
    parts: list[dict] = []
    if text:
...
def tool_result_message(name: str, result: Any) -> dict:
    """Gemini expects tool output back as a functionResponse from the user."""
    return {
        "role": "user",
...
__all__ = [
    ...
    "user_message", "model_message", "tool_result_message",
    "CHAT_MODEL", "BASE_URL",
]
